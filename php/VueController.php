...
//next主题添加以下内容
        $origin_next_config =  NextConfig::get();
        $next_config = [];
        foreach ($origin_next_config as $config) {
            $config['value'] = $config->getValue();
            $next_config[$config['key']] = $config;
        }
//next主题添加以上内容
        ...
        $res['globalConfig'] = array(
            ...
            'themes'                  => $themes,
            'next_config'             => $next_config,//next主题添加
